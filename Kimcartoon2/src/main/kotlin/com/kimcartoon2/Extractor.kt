...
// ============================================================
// GDMIRRORBOT - FULL CUSTOM EXTRACTOR
// ============================================================
...
// ============================================================
// STREAMRUBY - CUSTOM EXTRACTOR WITH REGEX
// ============================================================
open class Streamruby : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        Regex("file:\\s*\"(.*?m3u8.*?)\"").find(txt)?.groupValues?.getOrNull(1)?.let { m3u8 ->
            callback.invoke(
                newExtractorLink(name, name, url = m3u8, INFER_TYPE) {
...
// ============================================================
// AWSSTREAM - CUSTOM EXTRACTOR WITH API
// ============================================================
open class AWSStream : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        val response = app.post(m3u8Url, headers = header, data = formdata).parsedSafe<AWSStreamResponse>()
...
class ascdn21 : AWSStream() {
    override val name = "ascdn21"
    override val mainUrl = "https://as-cdn21.top"
    override val requiresReferer = true
}

// ============================================================
// ANIMEDEKHOCO - DIRECT LINK EXTRACTOR
// ============================================================
class Animedekhoco : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        text?.let {
            Regex("""file\s*:\s*"([^"]+)"""").find(it)?.groupValues?.get(1)?.let { link ->
        ...
        links.forEach { (serverName, serverUrl) ->
            callback.invoke(
                newExtractorLink(serverName, serverName, url = serverUrl, INFER_TYPE) {
                    this.referer = mainUrl
                    this.quality = Qualities.Unknown.value
                }
            )
...
// ============================================================
// STREAMRUBY (STREAMRUBY.COM variant)
// ============================================================
class StreamRuby : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        val response = app.get(cleanedUrl, referer = cleanedUrl, headers = mapOf("X-Requested-With" to "XMLHttpRequest")).document
        ...
        Regex("file:\"(.*)\"").find(scriptData)?.groupValues?.getOrNull(1)?.let { link ->
            callback.invoke(
                newExtractorLink(name, name, url = link, INFER_TYPE) {
                    this.referer = mainUrl
                    this.quality = Qualities.P1080.value
...
// ============================================================
// BLAKITEAPI - API-BASED EXTRACTOR
// ============================================================
...
// ============================================================
// ABYASS - ENCRYPTED DECRYPT EXTRACTOR
// ============================================================
open class Abyass : ExtractorApi() {
    ...
    override suspend fun getUrl(
        ...
        decrypted.sources.filter { it.status }.forEach { source ->
            callback.invoke(
                newExtractorLink(source = name, name = "$name [${source.codec.uppercase()}]", url = source.url, type = INFER_TYPE) {
    ...
    data class AbyssSource(val url: String, val size: Long, val type: String, val codec: String, val status: Boolean)
}
